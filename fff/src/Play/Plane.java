...
public class Plane extends MapElement implements Drawable {
	...
	public void shoot(int type,int number) {
		...
		if(fire){
			if(this.bulletStyle==Constants.FIRE_ONE){
				...
				bullets.add(b);
				new Music("src/music/fire.wav").playSound();
				...
				Bullet b=new Bullet(this.xpos,this.ypos,50,this.bulletStyle,bullets,false);
				bullets.add(b);
				//System.out.println("锟斤拷佣锟斤拷锟斤拷拥锟�");
			}	*/	
		}	
		}else{
			if(type==1){  //杩欓噷鐨則ype浠呮敼鍙樺瓙寮规暟閲�
			...
			else if(type==2){  //杩欓噷鐨則ype浠呮敼鍙樺瓙寮规暟閲�
				for(int i=0;i<number;i++)
				{
	...
	public void attackpress(KeyEvent e)//锟缴伙拷锟斤拷锟斤拷锟斤拷锟斤拷
	{
	...
	public void drawSelf(Graphics g) {
		if(this.isOver()&&neverover){
			if(this.style==Constants.ME){
				...
				new Music("src/music/boom.wav").playSound();
			...
			move();
				 ...
				 DrawString.draw(g, Color.red, "隶书", 40, 250, 450, "你挂了！生存时间："+endtime, 0);
				 new Music("src/music/boom.wav").playSound();
			}
		}	
	}
