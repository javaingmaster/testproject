...
{	
   ...
   // public boolean nevergetpoint=true;
			...
			List<Drawable> noticeAndExplosion)
	{
		...
		this.bgm=new Music("src/music/bgm.wav");
		bgm.playSound();
	}
	...
	public void paintComponent(Graphics g) 
	{
        ...
        if(scd.getthismonment()>=80&&scd.getthismonment()<=90){//+++ 画子弹，传参数
        	
        	if(ds.score>=70&&gift){
				ds.draw(g, Color.WHITE, "宋体", 40, 250, 450, "你的生命值有提升！", 0);
				me.life+=100;
				System.out.println("!!!!!!!!!!");
				gift=false;
			}
        ...
        if(scd.getthismonment()==90){         
        	ds.draw(g, Color.WHITE, "宋体", 40, 250, 450, "Boss！！！", 0);
        	RemoveSingleBulletList();
			RemoveEnemy();
        ...
        if(scd.getthismonment()>91){   	
        	...
        	if(enemy.size()==0){
        		me.life=-1;
        		ds.draw(g, Color.WHITE, "宋体", 40, 250, 450, "恭喜您成功生存！", 0);
        	}
        }
