...
public class Toolbag 
{
	...
	public static boolean regconize(Feature[] data,Feature imaged,int compareindex)
	{
		...
		for(int i=0;i<80;i++)
		{
			if(Math.abs(imaged.up[i]-data[compareindex].up[i])<=data[compareindex].up[i]*0.15)
			{
				count++;
			}
		}
		...
		{
			if(Math.abs(imaged.down[i]-data[compareindex].down[i])<=data[compareindex].down[i]*0.15)
			{
				count++;
			}
		}
		...
		{
			if(Math.abs(imaged.left[i]-data[compareindex].left[i])<=data[compareindex].left[i]*0.15)
			{
				count++;
			}
		}
		...
		{
			if(Math.abs(imaged.right[i]-data[compareindex].right[i])<=data[compareindex].right[i]*0.15)
			{
				count++;
			}
		}
		...
		if(data[compareindex].isdown&&data[compareindex].isup){
			System.out.println(data[compareindex].symbol+" is regconized!");System.out.println();  first=true; isrecognize=true; return isrecognize;
